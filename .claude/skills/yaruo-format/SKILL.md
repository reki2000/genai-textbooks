---
name: yaruo-format
description: やる夫式教材（docs/books/*/README.md）の公開前に、レンダリングが崩れる書式を機械的に検出・修正するlintスキル。日本語の約物と隣接して壊れる強調記号、区切り行の無い表、発言外枠のカギカッコ、発言末の句点、丸囲み数字（①②③）、メートル・グラム系単位のカタカナ表記を scripts/yaruo_lint.py で一括修正する。「整形」「フォーマット」「lint」「太字が効かない」「強調が崩れる」「表が崩れる」「カギカッコ」「句点」「丸囲み数字」「単位表記」に言及し、対象がやる夫式教材の .md なら使う。
---
