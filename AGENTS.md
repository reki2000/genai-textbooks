...
エントリに `draft: true` を付けると、その教材は公開の一覧（トップページ・サイドバー・サイトマップ）から外れたまま、本番と同じ経路でビルド・デプロイされる。サイトを `?draft=1` 付きで開けば一覧に現れ、公開するときは `draft: true` の1行を消す。仕様は [`BUILD.md`](./BUILD.md) の「下書き（draft）」を参照。
