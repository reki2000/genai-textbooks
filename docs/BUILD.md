...
- `catalog.yml` のエントリはパート数によらず1教材1エントリ（`path` は教材ディレクトリのまま）。`generate_site.py` がディレクトリ内の `README*.md` を自動検出してパート扱いするため、パート専用のカタログ登録は不要
- サイドバー・トップページの読了時間は合計を丸めず、各パートの分数を `+` でつないだ表記にする（例：`20+53+24分`）
- サイドバーには教材名の下に「I部」「II部」...のリンクが並び、それぞれ docsify の `subMaxLevel` 設定により、開いているパート内の見出し目次がそのリンクの下に自動表示される
