...
**やる夫**：
紙は全体では中性だから、近い側へどれだけ逆符号の電荷が偏れば持ち上がるかを逆算するお。
紙片の重さは、

$$mg=(1.0\times10^{-6})(9.8)
\approx9.8\times10^{-6}\,\mathrm N$$

近い側の偏りを点電荷 $q_{\mathrm{eff}}$ とみなせば、持ち上がる境目は、

$$q_{\mathrm{eff}}\approx
\frac{(9.8\times10^{-6})(3.0\times10^{-2})^2}
{(8.99\times10^9)(1.0\times10^{-8})}
\approx9.8\times10^{-11}\,\mathrm C$$

約 $1.0\times10^{-10}\,\mathrm C$ の偏りがあれば、静電力は紙片の重さと同じ $10^{-5}\,\mathrm N$ 程度になるお。
下敷きの電荷の約100分の1が近い側へ偏るだけで届く桁なのかお。

**やらない夫**：
実際の紙では分極した電荷が面に分布するため、今求めたのは点電荷近似で必要になる有効電荷だ。
本当にそれだけ偏るかは紙の形や材質、湿度を含めた測定が要るが、$3\,\mathrm{cm}$ で動くための条件は数量にできた。
