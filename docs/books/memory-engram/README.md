...
ニューロン全体と、その先端の1接点を並べると図2-1になるお。

[![細胞体近傍の樹状突起と遠くへ伸びる1本の軸索、その終末にあるシナプス](figs/fig1-neuron-overview.png)](figs/fig1-neuron-overview.png)

図2-1：多数に枝分かれする樹状突起は細胞体の近くで入力を受け、1本の軸索は遠位で側枝と終末を作る。拡大部では、送り手と受け手の膜がシナプス間隙を挟んで向き合う（模式図・縮尺不同、タップ／クリックで原寸表示）。
...
**やる夫**：
ここまでを一枚で見るお。
AMPA受容体からNaが入って受け手が脱分極し、その脱分極でNMDA受容体からCaが入る。まずはここまでだお。

[![Na流入による脱分極でNMDA受容体のMgの栓が抜け、Caが流入する2状態の模式図](figs/fig2a-nmda-gate.png)](figs/fig2a-nmda-gate.png)

図2-2：静止時はAMPA受容体からNaが入ってもNMDA受容体のMgの栓がCaを遮断する。脱分極時にはMgが抜け、NMDA受容体からCaが入る（タップ／クリックで原寸表示）。
...
図3-1なら、その一致検出から受容体が増えるまでを一続きで追えるお。

[![NMDA受容体の一致検出からCaMとCaMKIIを経てAMPA受容体が増える初期LTP](figs/fig2-early-ltp.png)](figs/fig2-early-ltp.png)

図3-1：グルタミン酸と受け手の脱分極が一致するとNMDA受容体のMg栓が外れ、Ca、CaM、CaMKIIの順に信号が進む。既存AMPA受容体のリン酸化と新たな膜挿入によってシナプスが強くなる（タップ／クリックで原寸表示）。
...
図3-2の左右で見れば、入口を変えずに結果だけが反転するのが分かるお。

[![高く短いCaはCaMKIIとLTPへ、低く長いCaはカルシニューリンとLTDへ分岐する](figs/fig3-ltp-ltd-branch.png)](figs/fig3-ltp-ltd-branch.png)

図3-2：海馬では同じNMDA受容体からのCa流入でも、高く短い信号はCaMKII、リン酸化、AMPA受容体挿入を経てLTPへ進み、低く長い信号はカルシニューリン、脱リン酸化、AMPA受容体回収を経てLTDへ進む（タップ／クリックで原寸表示）。
...
図4-1で、離れたスパインの信号が核の転写へつながる道筋も見通せるお。

[![スパインのCaとcAMPの信号がERKへ合流し、核のCREBを介して新しいタンパク質を作る](figs/fig4-late-ltp-nucleus.png)](figs/fig4-late-ltp-nucleus.png)

図4-1：活動スパインのCa・CaM経路とcAMP・PKA経路はMAPK経路へ合流し、ERKを介して核内のCREBを活性化する。CREBとCBPによる転写、mRNA、タンパク質合成が後期LTPを支える（タップ／クリックで原寸表示）。
...
合わせてシナプスタグ・キャプチャ仮説だ。
強い線と弱い線と活動しない線を同時に置いた図4-2なら、誰が荷物を受け取るかが見える。

[![強い活動と弱い活動でタグが付いたシナプスだけが新しいタンパク質を捕獲する](figs/fig5-synaptic-tagging.png)](figs/fig5-synaptic-tagging.png)

図4-2：強い活動Aが核で新しい可塑性関連タンパク質を作らせる。タンパク質は樹状突起を広く運ばれるが、タグのあるAと弱い活動Bだけが捕獲し、活動しなかったCでは通過する（タップ／クリックで原寸表示）。
...
図4-3は、核から運んだ設計図を現場で使う話と、Arcが受容体を回収する話を同じニューロン上に置いてるお。

[![核から運ばれたArc mRNAが樹状突起で局所翻訳され、AMPA受容体の回収を助ける](figs/fig6-local-translation-arc.png)](figs/fig6-local-translation-arc.png)

図4-3：核で転写されたmRNAは樹状突起へ運ばれ、活動シナプスの近くでArcへ局所翻訳される。ArcはAMPA受容体の回収を助け、細胞全体の活動が高いときには複数の接点を下げる恒常性調節にも働く（タップ／クリックで原寸表示）。
...
図7-1では、音と痛みの情報線とは別に、その号令の線が扁桃体へ入ってるお。

[![音と痛みが扁桃体へ収束し、ノルアドレナリンが別経路で固定化を修飾する](figs/fig8-amygdala-circuit.png)](figs/fig8-amygdala-circuit.png)

図7-1：音と痛みは外側扁桃体の同じニューロンへ収束し、同時活動によるNMDA受容体依存LTPが音の線を強める。青斑核からのノルアドレナリンはβアドレナリン受容体、cAMP・PKA、ERK、CREBを経て、別経路から長期固定化を修飾する（タップ／クリックで原寸表示）。
...
図7-2なら、三つの条件とD1からDARPP-32までの反応を一枚で追えるお。

[![前後細胞の活動による適格度トレースにドーパミンが加わると線条体の書き込みが確定する](figs/fig9-striatum-three-factor.png)](figs/fig9-striatum-three-factor.png)

図7-2：送り手と受け手の発火が活動シナプスに数秒の適格度トレースを残し、その間にドーパミンが届くと書き込みが確定する。D1受容体、cAMP・PKA、DARPP-32の経路はホスファターゼを抑え、リン酸化状態を消えにくくする（タップ／クリックで原寸表示）。
...
図8-1では、強い接点には印がなく、弱い接点だけに補体とミクログリアが集まってるお。

[![補体で標識された活動の弱いシナプスだけをミクログリアが認識して貪食する](figs/fig11-complement-pruning.png)](figs/fig11-complement-pruning.png)

図8-1：発達期には活動の強いシナプスが残る一方、活動の弱いシナプスには補体の印が付く。ミクログリアの補体受容体がその印を認識し、弱い接点を包み込んで選択的に貪食する（タップ／クリックで原寸表示）。
