...
**やる夫**：一般相対論を学び終えた学生。式の意味を直観で掴むことはあるが、すぐに最短ルートを発見したと思い込み、宇宙規模で事故を起こす。

**やらない夫**：物理学の教師。完成した宇宙論を教える代わりに、観測データと反例を突きつけ、やる夫自身に理論を組み立てさせる。

**できる子**：観測と数値解析に強い研究者。必要なときだけ現れ、短い言葉でデータの意味と限界を指摘する。

**できる夫**：理論物理に詳しく、新模型を思いつくと異常な速度で説明を始める。ほとんどの場合、自由パラメータを増やしすぎてやらない夫に止められる。
...
**やらない夫**：
計算結果を写す前に、どんな形が必要か予想しろ。
ニュートン宇宙論なら、膨張率の2乗は密度と曲率に関係しそうだ。
宇宙定数も加えればどうなる。

**やる夫**：
次元を合わせると $G\rho$ は時間の逆2乗。
$H^2$ と同じ次元だお。
曲率は $kc^2/a^2$、宇宙定数は $\Lambda c^2$。
係数を計算すると
...
**やらない夫**：
密接だが概念は違う。
再結合は自由電子が原子へ取り込まれて電離度が下がる過程。
脱結合は散乱率が膨張率を下回り、光子が物質と熱的・運動学的に結びつかなくなる過程だ。
電離度が少し残っていても、平均自由行程が十分長ければ光子は自由飛行できる。

**やる夫**：
その後、最初の星や銀河が紫外線を出して中性水素を再び電離する。
これが再電離だお。
名前が似ていて混乱するお。

**やらない夫**：
再電離で生じた自由電子はCMB光子の一部を再散乱する。
その確率を光学的深さ $\tau$ が表し、小スケール温度異方性を $e^{-2\tau}$ 程度抑え、大角度偏光を作る。

**やる夫**：
だからCMBから原始揺らぎ振幅 $A_s$ を測るとき、$A_se^{-2\tau}$ の縮退が生じる。
低 $\ell$ 偏光で $\tau$ を測らないと、現在の構造成長振幅までずれるお。
...
**やらない夫**：
ただし1つの宇宙しか観測できないため、低 $\ell$ では独立な $m$ の数が少ない。
理論平均 $C_\ell$ を無限に正確には測れない。

**やる夫**：
装置を良くしても消えない誤差……宇宙分散かお。
観測可能な空が1枚しかないことによる標本誤差だお。

**やらない夫**：
そうだ。
宇宙論では、測定ノイズをゼロにしても残る限界がある。
...
**やる夫**：
BAOの山は重力進化で壊れないのかお。
銀河が非線形に集まれば、初期の殻はぐちゃぐちゃになるお。

**やらない夫**：
完全には消えないが、ピークは広がり、位置もわずかにずれる。
大規模な変位場を銀河分布から推定し、過去の位置へ部分的に戻す再構成という手法で、BAO信号を鋭くできる。

**やる夫**：
観測データを勝手に理論通りへ直してるようにも見えるお。
...
**やる夫**：
線形成長を実際に観測するには、$\delta$ を直接測れないお。
銀河数はバイアスされ、暗黒物質は光らない。

**やらない夫**：
だから複数の代理量を使う。
銀河クラスタリングはバイアスを介して密度場を、赤方偏移空間歪みは速度場を、弱重力レンズは光路の積分された重力ポテンシャルを測る。
銀河団数は高密度尾に敏感だ。

**やる夫**：
一般相対論では、非相対論的物質が感じるポテンシャル $\Psi$ と、光が感じる $\Phi+\Psi$ の関係が決まる。
修正重力では両者がずれる可能性があるお。

**やらない夫**：
そのずれを重力スリップと呼ぶことがある。
銀河運動とレンズを組み合わせれば、見えない質量を足す模型と重力法則を変える模型を区別する手掛かりになる。
...
**やる夫**：
物質だけの減速宇宙と比べると、遠方超新星が予想より暗い。
光度距離が大きいということは、現在までの膨張履歴が物質だけの模型と違うお。
...
**やる夫**：
波長依存性で塵を調べ、近傍・遠方標本の性質を比較し、異なる観測装置を較正する。
さらに超新星だけで決めず、CMBとBAOの距離情報を組み合わせるお。
...
**やらない夫**：
初期宇宙推定と一部の局所距離梯子の間には、長く無視できない差が報告されている。
ただし「2つの装置が同じ量を直接測って食い違った」と単純化するな。
...
|---|---|
| 大尺度の背景膨張 | 高精度で説明 |
| 軽元素合成 | 重水素・ヘリウムは概ね整合、リチウム問題あり |
| CMB音響ピーク | 6パラメータ模型で高精度に説明 |
| BAO・銀河分布 | 大枠を説明 |
| 構造成長・弱レンズ | 概ね整合、解析依存の緊張あり |
| 加速膨張 | 宇宙定数で最小に説明 |
| 暗黒物質の微視的正体 | 不明 |
| 暗黒エネルギーの正体 | 不明 |
| インフレーションの具体機構 | 不明 |
