...
**やる夫**：一般相対論（general relativity）を学び終えた学生。式の意味を直観で掴むことはあるが、すぐに最短ルートを発見したと思い込む。

**やらない夫**：物理学の教師。

**できる子**：観測と数値解析（numerical analysis）に強い研究者。データの意味と限界の切り分けに厳しい。
...
計量（metric） $g_{\mu\nu}$ が時空の幾何を決め、アインシュタイン方程式（Einstein field equations）

$$G_{\mu\nu}+\Lambda g_{\mu\nu}=\frac{8\pi G}{c^4}T_{\mu\nu}$$

が物質と曲率を結ぶ。
星の周りならシュバルツシルト解（Schwarzschild solution）、回転していればカー解（Kerr solution）。
...
**やる夫**：
古い端末が5台あるお。
1号機は銀河の距離とスペクトル線（spectral line）のずれ。
2号機は水素、重水素（deuterium）、ヘリウム、リチウムの存在量。
...
横には光円錐（light cone）の形をした巻尺と、成分別の宇宙会計簿もあるお。
ずいぶん答えを教えてくれる装置だお。
...
なら全銀河の位置 $\mathbf x_i$、速度 $\mathbf v_i$、質量 $m_i$ を初期条件（initial conditions）として入力し、一般相対論的な $N$ 体計算（N-body simulation）をすればいい。
原理的には完全だお。
...
**やる夫**：
銀河を粗視化（coarse-graining）して、宇宙の平均密度 $\rho(t)$ を使うのかお。
でも宇宙は一様じゃないお。
地球、太陽、銀河、銀河団、空洞がある。
...
**できる子**：
全天銀河サーベイ（all-sky galaxy survey）。
...
**やらない夫**：
逃げ道ではなく階層化だ。
背景宇宙を一様・等方な零次近似として記述し、その上に銀河や空洞を摂動（perturbation）として載せる。
まず背景を解き、次に不均一性の成長を解く。
1度に全部解こうとしたお前の案を、問題の階層に分けたわけだ。

**やる夫**：
つまり第1段階では、どの場所も同等で、どの方向も同等と仮定する。
これが宇宙原理（cosmological principle）かお。
...
赤方偏移（redshift）を後退速度だと思えば、どの方向の銀河も地球から遠ざかっているお。
結論は1つ。
地球は宇宙大爆発の中心だお。
...
となる。
共動座標（comoving coordinate）の差 $\chi$ を固定して時間微分しろ。
...
だお。
距離に比例して後退速度が大きくなるハッブル則（Hubble's law）が、格子の一様な拡大だけで出たお。
...
特殊相対論（special relativity）違反だお。

**やらない夫**：
特殊相対論が禁止するのは、同じ局所慣性系（local inertial frame）で物体が光を追い越すことだ。
...
**やらない夫**：
よくつながったな。
宇宙膨張とは、物体が中心から空間を突っ切る運動ではなく、共動点間の計量距離を決める尺度因子（scale factor） $a(t)$ の変化だ。

---
### 1-3　対称性だけで計量を絞る

**やる夫**：
一様・等方なら、宇宙の計量は平坦なミンコフスキー（Minkowski）時空に $a(t)$ を掛ければいいお。
...
これがFLRW計量（Friedmann-Lemaître-Robertson-Walker metric）だ。
...
**やらない夫**：
宇宙の平均物質流とともに動く共動観測者（comoving observer）の固有時間（proper time）だ。
完全流体（perfect fluid）の4元速度に直交する空間面を、同じ宇宙時刻の面として使う。
...
**やる夫**：
銀河や暗黒物質（dark matter）は圧力ゼロの塵 $p\simeq0$。
光子は圧力を持つ。
真空（vacuum）エネルギーには負圧がある。
全部同じ器に入るお。
...
これが第1フリードマン（first Friedmann）方程式だお。自分で建てたお。
...
だ。
状態方程式（equation of state） $p=w\rho c^2$ なら $w<-1/3$ が必要になる。
...
**やる夫**：
宇宙会計監査を始めるお。
共変保存則（covariant conservation law）
...
時間並進対称性（time-translation symmetry）がなければ、ノーター（Noether）の定理の意味で保存する全エネルギーもない。
...
**やらない夫**：
粒子種が平衡から脱結合（decoupling）した後も、運動量は膨張で $p\propto a^{-1}$ に赤方偏移する。
ただし後から別の粒子が消滅して、結合している成分だけを加熱する場合、温度比は単純に1ではない。

**やる夫**：
電子・陽電子が対消滅（annihilation）すると、そのエントロピーは光子へ入り、先に脱結合したニュートリノには入りにくい。
...
これが後で平坦性問題（flatness problem）として戻ってくる。
...
$a\to0$ では密度、温度、曲率不変量（curvature invariant）が発散し、古典的一般相対論の適用が疑わしくなる。
特異点（singularity）は「無限密度の物体が実在した」という完成した描写ではない。
理論がそこで予測能力を失う印でもある。
...
でも特異点そのものの物理には量子重力（quantum gravity）が必要かもしれない。
...
**やらない夫**：
よい疑いだ。
座標成分ではなく曲率不変量や測地線完全性（geodesic completeness）を調べる。
...
**やらない夫**：
特異点定理（singularity theorem）は、適切なエネルギー条件、因果条件、十分な収束などから測地線不完全性を導く。
...
われわれが見られる範囲は粒子地平線（particle horizon）で限られるが、その外にも同じFLRW空間が続く可能性がある。
...
で速度が出る。
宇宙論的赤方偏移（cosmological redshift）も全部これでいいお。
...
を満たす $D_L$ を光度距離（luminosity distance）と呼ぶ。
膨張宇宙で共動距離との関係は。
...
を満たす角径距離（angular diameter distance） $D_A$ は。
...
**やらない夫**：
この関係は距離双対性（distance duality）と呼ばれ、光子数保存や光がヌル（null）測地線を進むことに依存する。
...
その通り。
地平線は計量の全履歴に依存する。
インフレーション（inflation）は、熱いビッグバン以前に共形時間（conformal time）を追加し、現在離れた領域を過去に因果接触させる機構として働く。
...
**やる夫**：
現在観測できる最遠方は粒子地平線なのに、電磁波で直接見える最古は最後の散乱面（last-scattering surface）だお。
...
その時点の存在量が凍結（freeze-out）して後世へ残る。
化石みたいだお。
...
観測者まで散乱されずに届く確率を光学的深さ（optical depth） $\tau$ で表せば、最後の散乱時刻は1点ではなく幅を持つ。

**やる夫**：
可視度関数（visibility function） $g(\eta)=\dot\tau e^{-\tau}$ が最大になる付近が最後の散乱面だお。
面と呼んでも時間幅がある。
...
$\tau$ を独立に決めないと、$A_s$ が決まらないお。**縮退（degeneracy）** だお。
...
宇宙分散（cosmic variance）が一番きつい場所で $\tau$ を測って、その $\tau$ で $A_s$ を割るんだお。
...
**やらない夫**：
全天の温度場を球面調和関数（spherical harmonics）で展開する。
...
2次元地図を1次元のパワースペクトル（power spectrum）へ圧縮するんだお。
...
**やる夫**：
各フーリエモード（Fourier mode） $k$ が振動子として
...
これが幾何学的縮退（geometrical degeneracy）だ。
...
スカラー密度摂動は主にEモードを作る。
テンソル摂動、つまり原始重力波はBモードを作りうるけど、後期宇宙の重力レンズ（gravitational lensing）もEをBへ変換するお。
...
**やる夫**：
4号機の銀河3次元分布を開いたお。
でも音響波は光子流体の現象だから、再結合後は消えて終わりだお。
銀河分布にまで痕跡が残るはずがないお。
...
2点相関関数（two-point correlation function）の小さな山として見えるお。
これがバリオン音響振動（baryon acoustic oscillations, BAO）かお。
...
そうすればぼけが取れて、山が鋭くなるお。再構成（reconstruction）だお。
...
**やらない夫**：
赤方偏移空間歪み（redshift-space distortions）は邪魔であると同時に、速度場と構造成長率を測る信号だ。
線形成長率
...
**やらない夫**：
そこで時間変化するスカラー場（scalar field） $\phi$ を考える。
一様場のエネルギー密度と圧力は
...
場がポテンシャルをゆっくり転がるスローロール（slow roll）だお。
...
**やる夫**：
十分な $e$ フォールド数（number of e-folds）
...
**やらない夫**：
インフレーション終了後、場のエネルギーを粒子へ移して宇宙を再加熱（reheating）する必要がある。
これで熱的ビッグバンの初期条件を作る。
...
単純な単一場スローロール模型では非ガウス性（non-Gaussianity）は小さい。
...
**やらない夫**：
それが等曲率、あるいはアイソカーバチャー（isocurvature）摂動だ。
全密度を変えずに成分比だけを変える。
...
**やらない夫**：
膨張宇宙で非相対論的物質の線形密度コントラスト（linear density contrast）
...
そのずれを重力スリップ（gravitational slip）と呼ぶんだお。
...
**やらない夫**：
5号機のIa型超新星（Type Ia supernova）データを見ろ。
...
超新星、CMB、BAOでは **主要な系統誤差の起源が違う** お。
3つが同じ方向を指したら、1つの系統誤差だけでは説明しにくいお。
...
これが $\Lambda$CDM（Lambda cold dark matter）だお。
...
第3は、時間遅延重力レンズ、メーザー、重力波の標準サイレン（standard siren）、TRGBやJAGBなど別の距離指標だお。
...
事後分布（posterior distribution）は尤度と事前分布（prior distribution）から得る。
...
**やる夫**：
模型比較ではベイズ（Bayes）因子、AIC、BICなどの考え方もあるけど、どれも万能の判定器じゃない。
事前分布や近似に依存するお。
...
| 宇宙年齢 | ── | 6パラメータから導かれ、最古の星・球状星団の年齢と矛盾しない |
| CMB温度 | $T_0=2.7255$ K | 熱平衡起源として説明 |
| 最終散乱 | $z_*\simeq1090$、等密度時 $z_{\rm eq}\simeq3400$ | 順序も含めて整合 |
| 軽元素合成 | $Y_p\simeq1/4$ | 重水素・ヘリウムはCMBの $\Omega_bh^2=0.0224$ と一致 |
| リチウム | 標準計算と観測が不一致 | **未解決** |
| CMB音響ピーク | 規則的なピーク列 | 平坦宇宙で説明 |
| BAO・銀河分布 | 殻半径150 Mpc級を複数の $z$ で共通に使用 | 整合（要再構成） |
| 構造成長・弱レンズ | $S_8$ | 概ね整合、解析依存の緊張あり |
...
**やる夫**：
……押さないお。
リチウムは標準計算と合ってないお。$H_0$ は $5\sigma$ だお。
