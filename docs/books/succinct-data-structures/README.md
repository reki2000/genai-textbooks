...
**やる夫**：ソフトウェア開発者。普通の配列、ハッシュテーブル、ポインタ木を使えば大抵の問題は解けると思っている。小さなテストデータで成功すると、すぐ世界制覇を確信する。

**やらない夫**：システム設計者。答えを直接教えず、必要容量や計算量をやる夫自身に計算させる。やる夫の実装が壊れるまで静かに待つこともある。

**できる子**：アルゴリズム担当。必要な事実だけを短く述べる。メモリ使用量やキャッシュミスを数字で突きつける。

**できる夫**：性能評価担当。大量のベンチマークを持ち込み、理論上の優劣と実機上の優劣が一致しないことを楽しそうに報告する。
...
**やらない夫**：
しかも、その計算にはオブジェクトヘッダ、アラインメント、参照、配列管理領域、ハッシュテーブルの空き領域が含まれていない。
...
**やらない夫**：
それを区別するには16ビット必要だ。
したがって、どんなビット列でも許されるなら16ビットは情報理論的に最小だ。
...
データの偏りや繰り返しを利用して小さくしたものを、圧縮データ構造と呼ぶ。
情報理論的下限に近い容量を使いながら、必要な演算を高速に実行するものを、簡潔データ構造と呼ぶ。
...
**やる夫**：
64ビット整数として読み、1の個数を数える `popcount` 命令を使えるお。
最大2ワードくらいで済むお。
...
S[\operatorname{super}(i)]
+
L[\operatorname{block}(i)]
+
\operatorname{popcount}(\operatorname{remainder}(i))
$$
...
**やる夫**：
整数列のランダムアクセスが、ビット列の `select` に変わったお。
...
また、ある値以上の最初の要素を探す predecessor や successor にも利用できる。
...
**やる夫**：
値が単調でないと上位部の並びが壊れるお。
値域 $U$ が要素数 $m$ に近い、つまり密な列なら、普通のビットベクトルの方が自然かもしれないお。
...
**やらない夫**：
ビット予算メーターに、容量だけでなく必要演算と応答時間を追加しろ。
...
$$
\log_2C_n\approx2n-\frac32\log_2n+O(1)
$$
...
**やらない夫**：
これがLOUDS、Level-Order Unary Degree Sequenceだ。
幅優先順の各ノードの次数を単項符号で並べている。
...
住所候補を階層順に表示するトライには向きそうだお。

**やらない夫**：
部分木全体を列挙したい場合はどうだ？

**やる夫**：
幅優先だと、文京区以下の小石川と本郷が、他の地区のノードと混ざる可能性があるお。
部分木が連続区間にならないお。
...
**やる夫**：
$$
\operatorname{excess}(i)
========================

\operatorname{rank}_1(i)-\operatorname{rank}_0(i)
$$
...
**やらない夫**：
`excess` に対する区間最小値や、指定値へ到達する位置を求めるための補助索引を置く。
...
* Balanced Parenthesesは深さ優先で、祖先や部分木操作に向く
...
**やらない夫**：
Wavelet Matrixでは、各階層で0の要素を前半、1の要素を後半へ安定的に並べ替える。
その階層の0の総数も記録する。
...
現代のCPUでは、メモリアクセスの局所性が支配的になることがある。
...
**やらない夫**：
それがBurrows–Wheeler変換、BWTの列 $L$ だ。
...
索引自体が元テキストの情報を保持するため、自己索引と呼ばれる。
...
**やる夫**：
動的ビットベクトルや動的Wavelet Treeを使えばいいお。
...
**やらない夫**：
簡潔構造が必要になりやすい条件をまとめろ。

**やる夫**：
まず規模だお。
数千万から数十億要素があり、1要素数バイトの差が数百MBから数十GBになる場合だお。

次に、メモリ上限がシステム成立条件になっていること。
端末に載らない、キャッシュに入らない、メモリ帯域が足りない、という問題がある場合だお。

データがほぼ静的であること。
更新が少ないほど、位置を固定した簡潔表現を使いやすいお。

必要演算を限定できること。
`access`、`rank`、`select`、範囲頻度、分位点、全文countなど、問い合わせを明確に定義できる場合だお。

データに単調性、疎性、偏り、繰り返しがあること。
Elias–Fano、RRR、FM-indexの効果が大きくなるお。

情報理論的下限との差を議論する価値があること。
単なる数%の節約ではなく、システムの可否が変わる場合だお。
...
| 課題                    | 有力な構造                | 主な条件          |
| --------------------- | -------------------- | ------------- |
| 高速なビット列 `rank/select` | Plain bitvector＋索引   | 容量より速度、偏りが小さい |
| 偏った静的ビット列             | RRR                  | 0または1に偏りがある   |
| 疎な位置集合、単調整数列          | Elias–Fano           | 値域が広く要素が疎     |
| 順序木、トライ               | LOUDS                | 幅優先操作、静的      |
| 祖先、部分木操作              | Balanced Parentheses | 深さ優先操作、静的     |
| 多値列の範囲頻度、分位点          | Wavelet Matrix       | 静的な整数列        |
| 圧縮全文検索                | FM-index             | count中心、静的文書  |
| 更新や集合演算を重視            | Roaringなど            | 理論下限より実用性能    |
| 大規模本体＋少量更新            | 静的簡潔構造＋差分層           | 更新を一時隔離できる    |
