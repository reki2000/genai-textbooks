...
1バイト配列なら100MBだお。
実装によって100倍近い差になるお！
1の割合を横に振って、1要素あたりの理論下限を描いてみるお。

[![1の割合が小さいほど1要素あたりの必要ビット数は1ビットより下がることを示す曲線](figs/fig1-1-entropy.svg)](figs/fig1-1-entropy.svg)

図1-1：横軸は1の割合。避難所の1%なら1要素0.081ビットで足りるのに、普通のビット列は1ビット払う。赤い面積がその差で、半々へ近づくほど差は消える（タップ／クリックで原寸表示）。
...
情報理論的下限（information-theoretic lower bound）に近い容量を使いながら、必要な演算を高速に実行するものを、簡潔データ構造（succinct data structure）と呼ぶ[^10]。
...
**やらない夫**：
それが `rank` の基本構造だ[^1]。
16ビットのやる市では二段階の効きが見えないから、48ビットに伸ばして描け。

[![大区画の累積と小区画の累積と残りのpopcountを足してrankを求める図](figs/fig2-1-rank.svg)](figs/fig2-1-rank.svg)

図2-1：`rank_1(31) = 6 + 5 + 1 = 12`。スーパーブロックを16ビット、ブロックを4ビットにした場合。索引に入っているのは大小2種類の累積値だけで、最後の3ビットはその場で数える（タップ／クリックで原寸表示）。
...
**やらない夫**：
簡潔データ構造ではよく現れる考え方だ[^5]。
例外的な区間は個別に扱い、典型的な区間を小さな索引で処理する。
標本を1の個数で刻むと何が起きるか、並べて見ろ。

[![1の個数で標本すると区間の長さが密度で桁違いになることを示す図](figs/fig2-2-select.svg)](figs/fig2-2-select.svg)

図2-2：標本は1を8個ごとに打つので、区間の長さは密度しだいで7倍以上ぶれる。同じ8個でも、長い区間と短い区間では割に合う持ち方が違う（タップ／クリックで原寸表示）。
...
復元できたお！
作るときと戻すときを、1枚に重ねて描いておくお。

[![上位部が上位ビット列の1の位置へ移りselectで戻ることを示す図](figs/fig3-1-elias-fano.svg)](figs/fig3-1-elias-fano.svg)

図3-1：上段が構築で、上位 $h$ を位置 $h+i$ の1にする。下段が復元で、`select` の結果から順位を引いて $h$ に戻す。橙の列が本文の $i=3$。上位部はどこにも整数として保存されていない（タップ／クリックで原寸表示）。

**やらない夫**：
これがElias–Fano表現（Elias–Fano representation）の中心だ[^3][^4]。
...
重要なのは、親子関係を `rank` と `select` に帰着できることだ[^1]。

**やる夫**：
ポインタを保存せず、ビット列中で何番目の0か、何番目の1かを数えれば移動できるお。
木とビット列を上下に並べると、対応が全部見えるお。

[![LOUDSの0がノードの終端、1が子に対応することを示す図](figs/fig4-1-louds.svg)](figs/fig4-1-louds.svg)

図4-1：同じ木を上下で見る。0の並び順がノード番号に、1の並び順が子のノード番号になっていて、ポインタは1本もない（タップ／クリックで原寸表示）。
...
さっきの木で `excess` を折れ線にしてみろ。

[![括弧列のexcessの折れ線の上で部分木が連続区間になることを示す図](figs/fig4-2-bp.svg)](figs/fig4-2-bp.svg)

図4-2：図4-1と同じ木を、今度は深さ優先で括弧にした場合。節3の開き括弧から、深さが同じ2へ初めて戻る位置までが部分木。折れ線がその間ずっと2より上にあることを見る（タップ／クリックで原寸表示）。
...
収容人数に至っては値域が数万で、
1億件×数万ビットなら、512MBどころか少なくとも100GBを超えるお。

しかも空の列が大量にできるお。
いまの12要素の玩具列へ自治体コード3000種を当てたら、出現できるのは最大12種だお。
残り2988本以上は**全部0のビット列**になるお。
列長を $n$ に戻しても、値域の各値へ一律に $n$ ビット払う無駄は同じだお。
...
だお。
12件の玩具列で、区間がどこへ写るか描いてみるお。

[![Wavelet Treeで区間が左右の子の区間へ写ることを示す図](figs/fig5-1-wavelet-tree.svg)](figs/fig5-1-wavelet-tree.svg)

図5-1：値域を上のビットから半分ずつ割る。区間 $[2,9)$ は左では $[2,6)$、右では $[0,3)$ へ移り、区間のまま値域だけが狭まる。ビット列がノードごとに切れている点に注目（タップ／クリックで原寸表示）。
...
各階層が1本の連続ビットベクトルになる。
ノードポインタは不要だ。
さっきと同じ列、同じ区間で並べ直して見ろ。

[![各階層が1本の連続ビット列になり1側の区間がZだけずれることを示す図](figs/fig5-2-wavelet-matrix.svg)](figs/fig5-2-wavelet-matrix.svg)

図5-2：図5-1と同じ列・同じ区間。各階層で0を前へ寄せると木の切れ目が消え、代わりに1側の区間が $Z_0$ だけ右へずれる。階層ごとに $Z_d$ を1個持てば、ポインタは要らない（タップ／クリックで原寸表示）。
...
**やらない夫**：
それがBurrows–Wheeler変換（Burrows–Wheeler transform, BWT）の列 $L$ だ[^8]。
...
**やる夫**：
また `rank` が出てきたお。
$F$ と $L$ だけ抜き出すと、対応がはっきりするお。

[![LとFで同じ文字の順序が保たれることを示すBWTの図](figs/fig6-1-bwt-lf.svg)](figs/fig6-1-bwt-lf.svg)

図6-1：左が辞書順に並べた巡回列、右はその先頭列 $F$ と末尾列 $L$ だけを抜いたもの。$L$ の1番目・2番目・3番目の `a` が、$F$ の1番目・2番目・3番目の `a` へ順序を保ったまま対応する。$C[a]=1$ は先頭の `$` の1行分（タップ／クリックで原寸表示）。
...
各文字につき2回の `rank` だお。
区間が3段階でどう動くか、さっきの表に重ねるお。

[![後方検索で接尾辞区間が3段階で狭まり件数が区間幅に出ることを示す図](figs/fig6-2-backward-search.svg)](figs/fig6-2-backward-search.svg)

図6-2：図6-1と同じ表。区間は狭まるだけでなく飛ぶ。`a` の区間から `na` の区間へは表の下方へ移り、`ana` で2行に落ち着く。ここまで原文も接尾辞配列も一度も参照していない（タップ／クリックで原寸表示）。
...
これがFerragina–Manzini索引（Ferragina–Manzini index, FM-index）の中核だ[^9]。
...
**やる夫**：
これがRRRビットベクトル（Raman–Raman–Rao bit vector）だお[^2]。
...
偏りのないビット列では、ほとんど縮まないのに処理だけ増える[^5]。
...
そして 54MiB 余るのは無駄じゃないお。
第8幕でやったとおり、この構造は更新できないお。
再構築までの間に増える施設と、ログと、OSが使う分だお。
**余白は無駄じゃなくて、更新できない構造が生きるための余白** だお。
4案を容量と応答の平面に置くと、一目で分かるお。

[![容量と応答時間の平面で最小容量の案が応答要件を越えることを示す図](figs/fig8-1-budget.svg)](figs/fig8-1-budget.svg)

図8-1：横軸が総容量、縦軸が検索応答。丸が中央値、棒の上端が最悪値。最小容量の圧縮中心版は中央値なら青い箱に入っているのに、棒が50msの線を突き抜ける（タップ／クリックで原寸表示）。
...
**やる夫**：
2MiB で 34ms 買えたお。
さっき「54MiB余らせるのは無駄じゃない」って偉そうに言ったばかりなのに、
