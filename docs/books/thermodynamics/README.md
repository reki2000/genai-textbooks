...
水の量は同じ。
最終温度もほぼ同じ。
ただしエンジンを通した方には、上に上がったワッシャーが残るお。
...
絶対零度でも量子的な話が残るはずだお。
……氷の秤が測るのは全エネルギーじゃなくて、決めた基準状態まで移せた差だけかお。
...
……同じなら、水が持つのは熱じゃなくて内部エネルギー。
熱は温度差によって境界を越えたエネルギーの呼び名なのかお。

**できる子**：
系が持つ状態量を内部エネルギー $U$ とする。
...
遠心分離機にかけて外側と内側に分ければ、熱い水と冷たい水に分かれるお。
...
真空へ自由膨張させたら、同じ体積まで広がっても重りは上がらない。
...
[![PV図の閉路で膨張仕事と圧縮仕事の差が正味仕事になる](figs/fig3-cycle-net-work.svg)](figs/fig3-cycle-net-work.svg)

図3：膨張だけでなく元へ戻る圧縮まで数え、閉路が囲む面積を一周の正味仕事として読む（タップ／クリックで原寸表示）。
...
冷却器で作動流体を低温へ戻し、高温熱源からまた熱を受け取れる状態にするお。
...
**やらない夫**：
分子の並び方を数える統計力学では、この同じ状態量が「実現できる微視的状態の数」と結びつく。
...
現実の装置なら止まる。
氷を作りながら余剰電力まで出す案は、第二法則に正面から負けたお。
...
**できる子**：
記録。
過冷却は製氷方式として利用もされるが、解除の制御に失敗すると配管閉塞を起こす。
...
**やる夫**：
ギブズ自由エネルギーが小さい方、と聞いたことはあるお。
でも式を暗記しただけで、なぜそれを比べるのかは説明できないお。

**やらない夫**：
一定の温度と圧力を保つ外界と接する系を考えよう。
単純な物質の可逆変化なら$dH=T\,dS+V\,dP$だ。
$G=H-TS$を自分で微分してみろ。
...
温度と圧力が一定なら、外界まで含めて自然に進む変化では$G$が減り、平衡で下げ止まる。
だから、ギブズ自由エネルギー

$$G=H-TS$$

が小さい相が安定なんだお。
純物質では、1mol増やしたときの$G$の変化が化学ポテンシャル$\mu$。
...
**やる夫**：
2相で化学ポテンシャルが等しい条件を微分すると、クラペイロンの式
...
[![同じ1MJでも電池、圧縮空気、高温熱、低温熱の順に最大仕事が異なる](figs/fig4-exergy-ranking.svg)](figs/fig4-exergy-ranking.svg)

図4：同じ1MJでも、環境35℃まで戻す間に取り出せる最大仕事は約1.00、0.82、0.60、0.016MJと大きく異なる（タップ／クリックで原寸表示）。
...
必要温度の高い用途から低い用途へ順に使うカスケード利用が有効。
...
でも装置が複雑になり、液滴を含む二相流、保守、コスト、制御の問題が出る。
熱力学は上限と損失場所を示す。
実際に採用するかは工学と経済の判断だお。
