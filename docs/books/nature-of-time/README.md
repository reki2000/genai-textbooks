...
**やらない夫**：
記録を見ている者と、記録の中の者の非対称性（asymmetry）から、未来の存在まで一気に飛んだわけか。
...
**やらない夫**：
前者は時計の状態変化として測れる。
後者は、独立した物理量（physical quantity）として何を測ればよいかが明らかでない。
...
何個でも……とは言えないお。
有限個なら普通の足し算が使える。
無限級数（infinite series）でも、1番目、2番目と番号を付けられる可算個なら、部分和の極限として定義できるお。

**やらない夫**：
「無限」を一種類だと思わなくなったな。では実数区間の点は、その可算個に収まるか。

**やる夫**：
カントールの対角線論法（Cantor's diagonal argument）では、実数は非可算（uncountable）だお。
番号を付けきれないお。
...
この長さの割り当てを測度（measure）と呼ぶ。

**やる夫**：
でも加法性に反してないかお。

**やらない夫**：
測度が要求する可算加法性（countable additivity）は、互いに重ならない可算個の集合についてだ。
...
**やる夫**：
なった、と言いたいお。
でも前の節で「集合と長さは別」とやったばかりだお。稠密（dense）さと連続も別かもしれないお。
...
**やらない夫**：
実数の重要な性質の一つが完備性（completeness）だ。[^3]
適切な意味で収束すべき列が、実数の外へ落ちない。

**やる夫**：
さらに実数区間は連結（connected）で、開集合（open set）を使って2つの離れた部分に分けられない。
有理数は稠密なのに、位相（topology）的には細かく分断されているお。
...
**やらない夫**：
「無限小（infinitesimal）の集まり」と「連続」は同じか。

**やる夫**：
同じ、と言うとまた「点が多ければ連続」の失敗を繰り返すお。
無限小は数の体系や解析の表現方法に関する概念で、連続性（continuity）は位相や順序、完備性に関する性質だお。
...
アルキメデス（Archimedean）の性質によれば、ある自然数 $n$ が存在して、
...
**やらない夫**：
標準解析（standard analysis）では、導関数（derivative）は
...
**やらない夫**：
現代には超実数（hyperreal number）を使う非標準解析（nonstandard analysis）がある。[^4]
そこでは正の無限小を厳密に扱える。
...
**やらない夫**：
プランク尺度（Planck scale）は、現在の理論をそのまま使うと量子重力（quantum gravity）を無視できなくなる目安だ。
時間の離散性を直接意味するとは限らない。
...
**やらない夫**：
このように連続で、逆写像（inverse map）も連続な対応を同相写像（homeomorphism）という。[^5]
...
次に微分構造（differentiable structure）。
滑らかな座標を使って速度や変化率を定義できるようにする。

次に計量（metric）。
時計が測る間隔を与える。

最後に因果構造（causal structure）。
...
**やる夫**：
閉じた時間的曲線（closed timelike curve）だお。
...
**やる夫**：
円の表と裏……ではないお。
各点で未来向きが連続につながるよう、時間の向き、時間指向（time orientation）を選ぶ必要があるお。
...
**やる夫**：
因果関係やエントロピー（entropy）増大、記憶が形成される向きと結びついてる。
でもそれは後で調べるお。
...
**やる夫**：
光信号を送ってアインシュタイン同期（Einstein synchronization）を行うお。
...
どちらも自分の慣性系（inertial frame）では正しいお。
離れた出来事の同時性は、観測者の運動状態に依存する。

**やらない夫**：
ローレンツ変換（Lorentz transformation）を書け。
...
となって、ニュートン重力ポテンシャル（Newtonian gravitational potential） $\Phi$ が時間成分に入るお。
...
「現在だけが存在する現在主義（presentism）」とすると、誰の現在を存在させるか決めにくいお。
...
これが永遠主義（eternalism）やブロック宇宙（block universe）と相対論の相性がよい理由だお。
...
現在だけが存在する現在主義。
過去と現在が存在し、未来はまだ存在しない成長ブロック宇宙（growing block universe）。
...
**やらない夫**：
今の区別を因果へ移せ。決定論（determinism）と宿命論（fatalism）も違う。
...
**やる夫**：
現在の出来事 $P$ へ影響できるのは、$P$ の過去光円錐（past light cone）内の出来事だお。
未来光円錐（future light cone）内の出来事は、$P$ から影響を受ける可能性はあるけど、通常は $P$ に影響できない。
...
**やる夫**：
ランダウアーの原理（Landauer's principle）では、論理的に不可逆（logically irreversible）な情報消去には最低でも、[^11]
...
孤立系（isolated system）のエントロピーは典型的に増大するお。
...
古典力学（classical mechanics）の多くも量子力学（quantum mechanics）の基本発展も、時間反転（time reversal）に対してほぼ対称だお。[^9]
...
**やらない夫**：
過去仮説（Past Hypothesis）だな。だが名前を付けただけでは使ったことにならん。
...
**やる夫**：
カオス系（chaotic system）では初期条件の微小差が指数的に増幅するお。[^12]

$$|\delta x(t)|\approx|\delta x(0)|e^{\lambda t}$$

$\lambda>0$ がリアプノフ指数（Lyapunov exponent）だお。
...
コペンハーゲン的な道具主義（instrumentalism）では。
...
**やらない夫**：
多世界解釈（many-worlds interpretation）では。
...
デコヒーレンス（decoherence）によって各成分が互いに干渉しにくくなり、観測者には一つの結果を経験する枝が生じるお。[^14]
...
**やらない夫**：
客観的収縮理論（objective collapse theory）なら。
...
**やる夫**：
同相じゃないお。
分岐点は局所的に1次元ユークリッド空間（one-dimensional Euclidean space）と同じ形ではない。

**やらない夫**：
通常の1次元多様体（one-dimensional manifold）になっているか。
...
量子論ならヒルベルト空間（Hilbert space）上の状態や、整合的歴史（consistent histories）の集合を扱えるお。
...
**やる夫**：
無意味じゃないお。
分岐時間論理（branching-time logic）や可能世界（possible world）、意思決定、並行プロセスのモデルとして役立つ。
...
観測者が変わっても、時間的または光的に結ばれた出来事の因果順序（causal order）は逆転しない。

**やらない夫**：
以前の失敗を踏み台にできたな。では第4幕で分けた位相と因果構造は、完全に無関係か。

**やる夫**：
十分条件のもとでは、時空の因果順序から位相や共形計量構造（conformal metric structure）の多くを復元できるお。[^16]
